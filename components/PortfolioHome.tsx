...
function ScrollReveal({
  ...
  return (
    <motion.div
      ref={ref}
      className={className}
      initial={prefersReducedMotion ? false : { opacity: 0, y: 20, filter: "blur(5px)" }}
      animate={prefersReducedMotion || isInView ? { opacity: 1, y: 0, filter: "blur(0px)" } : {}}
      transition={prefersReducedMotion ? { duration: 0 } : { duration: 0.65, delay, ease: [0.22, 1, 0.36, 1] }}
    >
      {children}
    </motion.div>
  );
